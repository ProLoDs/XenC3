/******************************************************************************
 * tools/pttrace/pttrace.c
 *
 * Tool for collecting pagetable buffer data from Xen.
 *
 * Copyright (C) 2004 by Intel Research Cambridge
 *
 * Author: Johann Betz, jbetz@hs-offenburg.de
 * Date:   February 2017
 */
...
static void disable_ptbufs(void)
{
    xc_interface *xc_handle = xc_interface_open(0,0,0);

    if ( !xc_handle ) 
    {
        perror("Couldn't open xc handle to disable ptbufs.");
        return;
    }
...
static void get_ptbufs(unsigned long *mfn, unsigned long *size)
{
    int ret;

    if(!opts.tbuf_size)
      opts.tbuf_size = DEFAULT_PTBUF_SIZE;

    ret = xc_tbuf_enable(xc_handle, opts.ptbuf_size, mfn, size);
...
static void event_init(void)
{
    ...
    rc = xc_evtchn_bind_virq(xce_handle, VIRQ_PTBUF);
    if (rc == -1) {
...
/**
 * monitor_ptbufs - monitor the contents of tbufs and output to a file
 * @logfile:       the FILE * representing the file to log to
 */
static int monitor_ptbufs(void)
{
    int i;
    ...
    get_ptbufs(&tbufs_mfn, &tinfo_size);
...
static void parse_args(int argc, char **argv)
{
                    ...
                    long_options, NULL)) != -1) 
    {
        ...
        case 'S': /* set ptbuf size (given in pages) */
            opts.ptbuf_size = argtol(optarg, 0);
            break;
