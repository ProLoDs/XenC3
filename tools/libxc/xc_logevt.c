 ...
 * xc_logevt.c
 *
 * API for manipulating and accessing log buffer parameters built on the trace buffer implementation
 *
 * Copyright (c) 2017, Johann Betz
 *
...
#include <xen/logevt.h>

static int logbuf_enable(xc_interface *xch, int enable)
{
    DECLARE_SYSCTL;
...
int xc_logbuf_set_size(xc_interface *xch, unsigned long size)
{
    DECLARE_SYSCTL;
...
int xc_logbuf_get_size(xc_interface *xch, unsigned long *size)
{
...
int xc_logbuf_enable(xc_interface *xch, unsigned long pages, unsigned long *mfn,
                   unsigned long *size)
{
    DECLARE_SYSCTL;
    int rc;
...
int xc_logbuf_disable(xc_interface *xch)
{
    return tbuf_enable(xch, 0);
}

int xc_logbuf_set_cpu_mask(xc_interface *xch, uint32_t mask)
{
    DECLARE_SYSCTL;
...
int xc_logbuf_set_evt_mask(xc_interface *xch, uint32_t mask)
{
    DECLARE_SYSCTL;
